...
export function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center noise-bg scanlines hatching-bg px-4 py-8" style={{ background: '#030303' }}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}
        className="w-full max-w-sm">

        {/* Logo */}
        <div className="text-center mb-8">
          <div className="font-display text-5xl sm:text-6xl mb-2" style={{
        ...
        <div className="border p-5 sm:p-6 space-y-4" style={{ background: 'rgba(10,10,10,0.95)', borderColor: 'rgba(255,255,255,0.07)' }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input className="input-dark" type="email" placeholder="trader@example.com" style={{ minHeight: 44 }}
                value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} required />
            </div>
            <div>
              <label className="label-mono">PASSWORD</label>
              <input className="input-dark" type="password" placeholder="••••••••" style={{ minHeight: 44 }}
                value={form.password} onChange={e => setForm(p => ({ ...p, password: e.target.value }))} required />
            </div>
            <button type="submit" disabled={loading}
              className="btn-acid w-full py-3.5 text-sm disabled:opacity-50" style={{ minHeight: 48 }}>
        ...
        <p className="text-center text-[8px] mt-6" style={{ color: 'rgba(255,255,255,0.1)', fontFamily: 'monospace' }}>
